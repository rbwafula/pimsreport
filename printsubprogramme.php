...
<html>
<head>
	<title>'.$division.' | PIMS+ Report</title>
	<!-- Vendor CSS -->
...
/*$pageheader = '<div class="row reportheader">
	            ...
	            <div class="col-md-4 title">
	                <h1> '.$processed_spdata[$division]["entity"].'</h1>
...
for ($i = 0; $i < count($processed_spdata[$division]["projectlisting"]); $i++) {
    ...
    $annex1table .= '<td class="left">' . $processed_spdata[$division]["projectlisting"][$i]['division'] . '</td>';
    $annex1table .= '<td class="left">' . $processed_spdata[$division]["projectlisting"][$i]['branch'] . '</td>';
    $annex1table .= '<td class="left">' . $processed_spdata[$division]["projectlisting"][$i]['project_id'] . '</td>';
    $annex1table .= '<td class="left">' . $processed_spdata[$division]["projectlisting"][$i]['project_title'] . '</td>';

    if ($processed_spdata[$division]["projectlisting"][$i]['months_remaining'] < 0) {
        $annex1table .= '<td class="center" style="color:#dc3545; font-weight: 500;">' . abs($processed_spdata[$division]["projectlisting"][$i]['months_remaining']) . '</td>';
    ...
    $annex1table .= '<td class="left">' . $processed_spdata[$division]["projectlisting"][$i]['project_manager'] . '</td>';
    $annex1table .= '<td class="right">' . number_format($processed_spdata[$division]["projectlisting"][$i]['budget'], 0, '.', ',') . '</td>';
    $annex1table .= '<td class="center"><img src="assets/images/'.gethealthimage($processed_spdata[$division]["projectlisting"][$i]['system_rating']).' " style="vertical-align:middle; height:12px;"></td>';
    $annex1table .= '<td class="center"><img src="assets/images/'.gethealthimage($processed_spdata[$division]["projectlisting"][$i]['management_rating']).' " style="vertical-align:middle; height:12px;"></td>';
    $annex1table .= '<td class="center"><img src="assets/images/'.gethealthimage($processed_spdata[$division]["projectlisting"][$i]['final_rating']).' " style="vertical-align:middle; height:12px;"></td>';
    $annex1table .= '<td class="center">' . $processed_spdata[$division]["projectlisting"][$i]['project_rank'] . '</td>';
    $annex1table .= '<td class="center">' . $processed_spdata[$division]["projectlisting"][$i]['outputs'] . '</td>';
    if ($processed_spdata[$division]["projectlisting"][$i]['completed_activities'] != '' && $processed_spdata[$division]["projectlisting"][$i]['total_activities'] != '') {
        $annex1table .= '<td class="center">' . $processed_spdata[$division]["projectlisting"][$i]['completed_activities'] . ' / ' . $processed_spdata[$division]["projectlisting"][$i]['total_activities'] . ' </td>';
    } else {
...
for ($i = 0; $i < count($processed_spdata[$division]["stafflisting"]); $i++) {
    if ($processed_spdata[$division]["stafflisting"][$i]['position_status'] == 'VACANT') {
        ...
        $annex2table .= '<td>' . $processed_spdata[$division]["stafflisting"][$i]['grade'] . '</td>';
        $annex2table .= '<td>' . $processed_spdata[$division]["stafflisting"][$i]['position_title'] . '</td>';
        $annex2table .= '<td>' . $processed_spdata[$division]["stafflisting"][$i]['position_number'] . '</td>';
        $annex2table .= '<td>' . $processed_spdata[$division]["stafflisting"][$i]['duty_station'] . '</td>';
        $annex2table .= '<td>' . $processed_spdata[$division]["stafflisting"][$i]['category'] . '</td>';
        $annex2table .= '<td>' . $processed_spdata[$division]["stafflisting"][$i]['org_code'] . '</td>';
        $annex2table .= '<td>' . $processed_spdata[$division]["stafflisting"][$i]['org_unit_description'] . '</td>';
        $annex2table .= '</tr>';
        $j++;
    }
}
...
for ($i = 0; $i < count($processed_spdata[$division]["stafflisting"]); $i++) {
    if ($processed_spdata[$division]["stafflisting"][$i]['position_status'] == 'FILLED') {
        ...
        $annex3table .= '<td>' . $processed_spdata[$division]["stafflisting"][$i]['grade'] . '</td>';
        $annex3table .= '<td>' . $processed_spdata[$division]["stafflisting"][$i]['position_title'] . '</td>';
        $annex3table .= '<td>' . $processed_spdata[$division]["stafflisting"][$i]['position_number'] . '</td>';
        $annex3table .= '<td>' . $processed_spdata[$division]["stafflisting"][$i]['duty_station'] . '</td>';
        //$annex3table .= '<td>' . $processed_spdata[$division]["stafflisting"][$i]['fund'] . '</td>';
        $annex3table .= '<td>' . $processed_spdata[$division]["stafflisting"][$i]['staff_name'] . '</td>';
        $annex3table .= '<td>' . $processed_spdata[$division]["stafflisting"][$i]['org_code'] . '</td>';
        $annex3table .= '<td>' . $processed_spdata[$division]["stafflisting"][$i]['org_unit_description'] . '</td>';
        $annex3table .= '</tr>';
        $j++;
    }
}
	...
	<div class="container-fluid printlandscape">
		<div id="toprint" class="toprint">
			<div class="row reportheader">
                ...
                <div class="col-md-4 title">
                    <h1><?php echo $processed_spdata[$division]["entity"]; ?></h1>
                    <h6>Programme Delivery Report</h6>
                </div>
                <div class="col-md-4 health">
                    <p class="reportdate">Jan 2021</p>
                    ...
                    <table class="table table-striped table-sm">
                        ...
                        <tbody>
							...
							for ($i = 0; $i < count($processed_spdata[$division]["projectlisting"]); $i++) {
							    ...
							    echo '<td class="center">SP ' . $processed_spdata[$division]["projectlisting"][$i]['sp_number'] . '</td>';
